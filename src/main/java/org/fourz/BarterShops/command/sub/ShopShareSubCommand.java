...
/**
 * Adds a co-owner to a shop group.
 * Usage: /shop share <groupId> <player>
 * Console-friendly: Yes (with admin permissions)
 */
public class ShopShareSubCommand implements SubCommand {

    private final BarterShops plugin;
    private final IShopGroupService groupService;

    public ShopShareSubCommand(BarterShops plugin, IShopGroupService groupService) {
        this.plugin = plugin;
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        groupService.addCoOwner(groupId, requester, coOwner).thenAccept(success -> {
            if (success) {
                sender.sendMessage(ChatColor.GREEN + "+ " +
                    plugin.getPlayerLookup().getPlayerName(coOwner) +
                    " added as co-owner of group #" + groupId);
            } else {
                sender.sendMessage(ChatColor.RED + "x Failed to add co-owner (not owner, group full, or invalid group).");
            }
        });
    ...
    public String getDescription() {
        return "Add a co-owner to a shop group";
    }

    @Override
    public String getUsage() {
        return "/shop share <groupId> <player>";
    }

    @Override
    public boolean hasPermission(CommandSender sender) {
        return sender.hasPermission(getPermission()) || sender.hasPermission("bartershops.admin") || sender.isOp();
    }
