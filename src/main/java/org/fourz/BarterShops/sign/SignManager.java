...
public class SignManager implements Listener {
    ...
    public void loadSignsFromDatabase() {
        ...
        plugin.getShopRepository().findAllActive().thenAccept(shops -> {
            int loaded = 0;
            for (ShopDataDTO shop : shops) {
                Location signLoc = shop.getSignLocation();
                if (signLoc == null || signLoc.getWorld() == null) {
                    logger.debug("Skipping shop " + shop.shopId() + " - sign location not resolvable");
                    continue;
                }

                Block block = signLoc.getBlock();
                if (!(block.getState() instanceof Sign)) {
                    logger.debug("Skipping shop " + shop.shopId() + " - block at location is not a sign");
                    continue;
                }
            ...
            logger.info("Hydrated " + loaded + " barter signs from database");
        }).exceptionally(ex -> {
            logger.error("Failed to load signs from database: " + ex.getMessage());
            return null;
        });
    }
